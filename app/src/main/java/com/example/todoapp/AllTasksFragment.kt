...
class AllTasksFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Handle FAB click to open AddTaskActivity
        fabAddTask.setOnClickListener {
            val intent = Intent(context, AddTaskActivity::class.java)
            startActivity(intent)  // Start the AddTaskActivity to create a new task
        }

        // Load data only if it hasn't been loaded already
        if (!isDataLoaded) {
            fetchTasksFromFirebase()
            isDataLoaded = true
        }
        ...
        if (userId != null) {
            db.collection("tasks")
                ...
                .addSnapshotListener { snapshots, error ->
                    ...
                    taskList.sortBy { it.deadline ?: Long.MAX_VALUE } // Tasks without a deadline go to the bottom
